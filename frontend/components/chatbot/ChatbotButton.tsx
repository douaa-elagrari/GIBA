...
async function getClient() {
  if (!_client) _client = await Client.connect(HF_SPACE);
  return _client;
}
...
export default function ChatbotButton() {
  ...
  const suggestedQuestions = [
    t.chatbot.suggestedQ1,
    t.chatbot.suggestedQ2,
    t.chatbot.suggestedQ3,
    t.chatbot.suggestedQ4,
  ].filter(Boolean);
  ...
  const handleSend = async (text?: string) => {
    ...
    setLoading(true);
    try {
      const answer = await callChatApi(query);
  ...
  const windowStyle: React.CSSProperties = isExpanded
    ? { ...S.window, width: '100vw', height: '100vh', borderRadius: 0, position: 'fixed', inset: 0, zIndex: 1001 }
    : S.window;
      ...
      {open && (
        <div className="hr-window-anim" style={windowStyle}>
          ...
          <div style={S.header}>
            ...
            <div style={{ flex: 1 }}>
              <p style={S.title}>{t.chatbot.title}</p>
              <div style={S.statusRow}>
                <div style={S.statusDot} />
                <p style={S.sub}>{t.chatbot.online}</p>
              </div>
            </div>
          ...
          <div className="hr-msgs" style={S.msgs}>
            ...
            {warming && (
              <p style={S.warm}>Space is warming up — first reply may take ~30s</p>
            )}
            <div ref={bottomRef} />
          </div>

          {/* Suggestions */}
          {showSugg && suggestedQuestions.length > 0 && (
...
const S: Record<string, React.CSSProperties> = {
  ...
  input:      { flex: 1, border: '1px solid #e5e7eb', borderRadius: 10, padding: '9px 14px', fontSize: 13.5, color: '#1f2937', background: '#f8fafc', transition: 'all 0.15s', fontFamily: "'DM Sans',sans-serif" },
